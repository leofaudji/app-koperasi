...
const AkunPage = {
    getKelompok(a) {
        const text = `${a.kode || ''} ${a.nama || ''}`.toLowerCase();
        if (a.tipe === 'aset') {
            if (/(penyertaan|investasi|saham|surat berharga|modal jangka panjang|sekuritas)/.test(text)) return 'Penyertaan Modal Jangka Panjang';
            if (/(tanah|bangunan|gedung|kendaraan|mobil|motor|peralatan|mesin|inventaris|aktiva tetap|furniture|perlengkapan|aset tetap|kenderaan)/.test(text)) return 'Aktiva Tetap';
            return 'Aktiva Lancar';
        }
        if (a.tipe === 'kewajiban') {
            if (/(jangka panjang|pinjaman|obligasi|leasing|hipotek|kewajiban jangka panjang|utang jangka panjang)/.test(text)) return 'Jangka Panjang';
            return 'Jangka Pendek';
        }
        if (a.tipe === 'modal') return 'Modal';
        if (a.tipe === 'pendapatan' || a.tipe === 'beban') return 'Laba/Rugi';
        return '-';
    },
    ...
    async render(container) {
        ...
        container.innerHTML = `<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fadeIn">
            ...
            <p class="text-xs text-gray-500 mb-4">Kelompok akun disimpan di database dan dipakai untuk laporan neraca.</p>
            <div class="table-wrapper"><table class="data-table w-full text-sm"><thead><tr class="bg-gray-50"><th class="px-4 py-3 text-left font-medium text-gray-500">Kode</th><th class="px-4 py-3 text-left font-medium text-gray-500">Nama Akun</th><th class="px-4 py-3 text-center font-medium text-gray-500">Tipe</th><th class="px-4 py-3 text-left font-medium text-gray-500">Kelompok</th><th class="px-4 py-3 text-center font-medium text-gray-500">Saldo Normal</th><th class="px-4 py-3 text-center font-medium text-gray-500">Aksi</th></tr></thead>
            <tbody>${res.data.map(a => `<tr class="border-t border-gray-50"><td class="px-4 py-3 font-mono font-bold">${a.kode}</td><td class="px-4 py-3 font-medium">${a.nama}</td><td class="px-4 py-3 text-center"><span class="badge ${tipeColors[a.tipe] || 'badge-info'}">${a.tipe}</span></td><td class="px-4 py-3 text-sm text-gray-600">${this.getKelompok(a)}</td><td class="px-4 py-3 text-center font-mono">${a.saldo_normal}</td>
    ...
    form(id = null) {
        const kelompokOptions = ['','Aktiva Lancar','Penyertaan Modal Jangka Panjang','Aktiva Tetap','Jangka Pendek','Jangka Panjang','Modal','Laba/Rugi'];
        ...
        <form id="akun-form" class="space-y-4"><input type="hidden" id="af-id" value="${id || ''}">
            ...
            <div><label class="block text-sm font-medium text-gray-600 mb-1">Saldo Normal</label><select id="af-sn" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm"><option value="D">Debit</option><option value="K">Kredit</option></select></div>
            <div class="col-span-2"><label class="block text-sm font-medium text-gray-600 mb-1">Kelompok</label><select id="af-kelompok" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm">${kelompokOptions.map(v => `<option value="${v}" ${v === '' ? 'selected' : ''}>${v || '-- Otomatis --'}</option>`).join('')}</select></div></div>

...
const LaporanPinjamanPage = {
    ...
    async render(container) {
        ...
        container.innerHTML = `<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-fadeIn">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                ...
                <div class="flex items-center gap-2">
                    <button onclick="LaporanPinjamanPage.export('pdf')" class="bg-red-50 text-red-600 hover:bg-red-100 px-3 py-2 rounded-xl text-sm font-medium transition-colors">
                        <i class="ri-file-pdf-line mr-1"></i> PDF
                    </button>
                    <button onclick="LaporanPinjamanPage.export('csv')" class="bg-emerald-50 text-emerald-600 hover:bg-emerald-100 px-3 py-2 rounded-xl text-sm font-medium transition-colors">
                        <i class="ri-file-excel-line mr-1"></i> CSV
                    </button>
                    <div class="w-px h-6 bg-gray-100 mx-1"></div>
                    <button onclick="LaporanPinjamanPage.load()" class="text-primary-600 hover:text-primary-700 p-2 rounded-lg hover:bg-primary-50 transition-colors" title="Refresh">
    ...
    async load() {
        ...
        this.data = res.data;
        const totalPinjaman = res.data.reduce((a, b) => a + parseFloat(b.total_pinjaman), 0);
        const totalTerbayar = res.data.reduce((a, b) => a + parseFloat(b.total_terbayar), 0);
        const totalSisa = res.data.reduce((a, b) => a + parseFloat(b.sisa_pinjaman), 0);
        ...
        const html = `<div class="table-wrapper">
            <table class="data-table w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    `).join('')}
                    ${res.data.length === 0 ? '<tr><td colspan="6" class="text-center py-10 text-gray-400">Tidak ada data pinjaman</td></tr>' : ''}
                </tbody>
                ${res.data.length > 0 ? `
